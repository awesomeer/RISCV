...
`define STACK_BASE 32'h8000_fff0
...
(
    ...
    initial begin
            ...
            reg_file[0] <= 32'd0;   reg_file[1] <= 32'd0;    reg_file[2] <= 32'd0;   reg_file[3] <= 32'd0;
    ...
    assign rs1 = reg_file[rs1i];
    assign rs2 = reg_file[rs2i];
...
(
    ...
    wire start = top.cpu.EXE_V && (top.cpu.EXE_OP == `INSTR_OP) && (top.cpu.EXE_F7 == 7'b0000001) && top.cpu.EXE_F3[2];
    ...
    always @(posedge clk) begin
        ...
        case(dcon)
            ...
            2'b01: begin
                if(a_ready[top.cpu.EXE_F3[0]] && b_ready[top.cpu.EXE_F3[0]]) dcon <= 2'b10;
                else dcon <= 2'b01;
            end
    ...
    assign dstall = start && !(dcon == 2'b10 && |done);
    Dividers dividers(
                        .s_axis_dividend_tdata(rs1),
                        .s_axis_dividend_tready(a_ready[0]),
                        .s_axis_dividend_tvalid(valid && a_ready[0]),
                        
                        .s_axis_divisor_tdata(rs2),
                        .s_axis_divisor_tready(b_ready[0]),
                        .s_axis_divisor_tvalid(valid && b_ready[0]),
                        
                        .m_axis_dout_tdata(signed_div),
                        .m_axis_dout_tvalid(done[0]),
                        
                        .aclk(clk),
                        .aclken(start && !top.cpu.EXE_F3[0])
                   );
                   
    Divideru divideru(
                        .s_axis_dividend_tdata(rs1),
                        .s_axis_dividend_tready(a_ready[1]),
                        .s_axis_dividend_tvalid(valid && a_ready[1]),
                        
                        .s_axis_divisor_tdata(rs2),
                        .s_axis_divisor_tready(b_ready[1]),
                        .s_axis_divisor_tvalid(valid && b_ready[1]),
                        
                        .m_axis_dout_tdata(unsigned_div),
                        .m_axis_dout_tvalid(done[1]),
...
(
    ...
    wire mul_start;
    signed_mul      ss_mul( .P(ss), .A(rs1), .B(rs2), .CLK(clk), .CE(mul_start && f3 == 3'b000));
    signed_mulh      ss_mulh( .P(ssh), .A(rs1), .B(rs2), .CLK(clk), .CE(mul_start && f3 == 3'b001));
    unsigned_mulh    uu_mul( .P(uuh), .A(rs1), .B(rs2), .CLK(clk), .CE(mul_start && f3 == 3'b010));
    signed_unsigned_mulh su_mul( .P(suh), .A(rs1), .B(rs2), .CLK(clk), .CE(mul_start && f3 == 3'b011));
    ...
    reg [1:0] state;
    reg [3:0] count;
    assign mul_start = top.cpu.EXE_V && (top.cpu.EXE_OP == `INSTR_OP) && (top.cpu.EXE_F7 == 7'b0000001) && !top.cpu.EXE_F3[2];
